 ...
 *   impl-guard = <MACRO>          required iff an `implementation` section
 *                                 is used (see below); wraps that section in
 *                                 #ifdef <MACRO> ... #endif, stb-style, so
 *                                 consumers get it only where they
 *                                 #define <MACRO> before including.
 *
 *   declarations                  switch subsequent include/embed entries
 *   implementation                to this section (default: declarations).
 *                                 declarations are always emitted;
 *                                 implementation is wrapped in impl-guard.
 *                                 System #includes are still hoisted/deduped
 *                                 to one top-level block regardless of which
 *                                 section pulled them in.
 *
...
typedef enum { SECTION_DECL, SECTION_IMPL } Section;
...
typedef struct
{
    ...
    const char *impl_guard;  /* NULL -> no #ifdef/#endif around SECTION_IMPL */
    Entry *entries;
...
static void manifest_load(Manifest *m, const char *path, Arena *a)
{
    ...
    for (char *line = strtok(text, "\n"); line; line = strtok(NULL, "\n")) {
        ...
        if (strcmp(t, "declarations") == 0) { section = SECTION_DECL; continue; }
        if (strcmp(t, "implementation") == 0) { section = SECTION_IMPL; continue; }
        ...
        if (eq) {
            *eq = 0;
            ...
            else if (strcmp(key, "impl-guard") == 0) m->impl_guard = arena_strdup(a, val);
            else {
        ...
        fprintf(stderr,
                ...
                "'embed <path> [name=] [mode=]', 'key = value', "
                "or 'declarations'/'implementation'\n",
                path, lineno);
        exit(1);
    }
}
...
int main(int argc, char **argv)
{
    ...
    int have_impl_entries = 0;

    for (int i = 0; i < m.n_entries; i++) {
        Entry *e = &m.entries[i];
        StrBuf *body = e->section == SECTION_IMPL ? &impl_body : &decl_body;
        if (e->section == SECTION_IMPL) have_impl_entries = 1;
    ...
    if (have_impl_entries && !m.impl_guard) {
        fprintf(stderr,
                "singleh: %s: 'implementation' section used but no "
                "'impl-guard = <MACRO>' set\n",
                manifest_path);
        return 1;
    }
    ...
    fwrite(decl_body.data, 1, decl_body.len, out);
    if (impl_body.len > 0) {
        fprintf(out, "\n#ifdef %s\n\n", m.impl_guard);
        fwrite(impl_body.data, 1, impl_body.len, out);
        fprintf(out, "\n#endif /* %s */\n", m.impl_guard);
    }
