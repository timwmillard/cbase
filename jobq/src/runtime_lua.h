const char runtime_lua_data[] =
    ...
    "-- dispatch. Embedded into the binary (see CMakeLists.txt / "
    "runtime_lua.h)\n"
    "-- and executed in each worker's lua_State, after the `jobq` module is\n"
    "-- published and before the user's handlers script (-l PATH) loads, so "
    "that\n"
    ...
    "}\n"
    "\n"
    "-- Canonical JSON: sorted keys, so equal args => equal bytes. Uniqueness "
    "is\n"
    "-- byte-comparison; cjson.encode does NOT sort keys. Use this when "
    "building\n"
    ...
    "    table.sort(keys, function(a, b) return tostring(a) < tostring(b) "
    "end)\n"
    "    local parts = {}\n"
    "    for i, k in ipairs(keys) do\n"
    "      parts[i] = cjson.encode(tostring(k)) .. \":\" .. "
    "canonical_json(v[k])\n"
    "    end\n"
    "    return \"{\" .. table.concat(parts, \",\") .. \"}\"\n"
    "  end\n"
