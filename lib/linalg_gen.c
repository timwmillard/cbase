...
Usage:
  cc -o linalg_gen linalg_gen.c && ./linalg_gen > linalg.h
  cc -o linalg_gen linalg_gen.c && ./linalg_gen --test > linalg_test.c
*/
...
static VecDef vecs[] = {
    {"bvec2", "uint8_t", 2, 0},  {"bvec3", "uint8_t", 3, 0},
    {"bvec4", "uint8_t", 4, 0},  {"ivec2", "int32_t", 2, 0},
    {"ivec3", "int32_t", 3, 0},  {"ivec4", "int32_t", 4, 0},
    {"uvec2", "uint32_t", 2, 0}, {"uvec3", "uint32_t", 3, 0},
    {"uvec4", "uint32_t", 4, 0}, {"vec2", "float", 2, 1},
    {"vec3", "float", 3, 1},     {"vec4", "float", 4, 1},
};
...
static void emit_test_header(void) {
   printf("/* generated — do not edit; run: ./linalg_gen --test > "
          "linalg_test.c */\n\n");
   ...
   printf("#define PASS(name) printf(COLOR_GREEN \"  PASS\" COLOR_RESET \"  "
          "%%s\\n\", name)\n\n");
   printf("static int _failed = 0;\n\n");
   printf("#define ASSERT(expr) do { \\\n");
   printf("    if (!(expr)) { \\\n");
   printf("        printf(COLOR_RED \"  FAIL\" COLOR_RESET \"  %%s:%%d  \" "
          "#expr \"\\n\", __FILE__, __LINE__); \\\n");
   printf("        _failed++; \\\n");
   printf("    } \\\n");
   printf("} while(0)\n\n");
...
static void emit_test_vec(VecDef *v) {
   ...
   if (v->is_float) {
      printf("    %s a = {", nm);
      for (int i = 0; i < n; i++)
         printf("%s%.1ff", i ? ", " : "", (float)a[i]);
      printf("};\n");
      printf("    %s b = {", nm);
      for (int i = 0; i < n; i++)
         printf("%s%.1ff", i ? ", " : "", (float)b[i]);
      printf("};\n\n");
   } else {
      printf("    %s a = {", nm);
      for (int i = 0; i < n; i++)
   ...
   if (n == 3) {
      printf("\n");
      if (v->is_float) {
         printf("    %s cx = %s_cross((%s){1.0f,0.0f,0.0f}, "
                "(%s){0.0f,1.0f,0.0f});\n",
                nm, nm, nm, nm);
         printf("    ASSERT_FEQ(cx.x, 0.0f); ASSERT_FEQ(cx.y, 0.0f); "
                "ASSERT_FEQ(cx.z, 1.0f);\n");
      } else {
...
static void emit_test_main(void) {
   ...
   printf("        printf(\"\\n\" COLOR_BOLD COLOR_GREEN \"All tests passed.\" "
          "COLOR_RESET \"\\n\\n\");\n");
   printf("    else\n");
   printf("        printf(\"\\n\" COLOR_BOLD COLOR_RED \"%%d test(s) failed.\" "
          "COLOR_RESET \"\\n\\n\", _failed);\n");
