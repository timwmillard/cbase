...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
   ...
   SDL_SetAppMetadata("Example Pen Drawing Lines", "1.0",
   ...
   if (!SDL_CreateWindowAndRenderer("examples/pen/drawing-lines", 640, 480, 0,
                                    &window, &renderer)) {
...
SDL_AppResult SDL_AppEvent(void *appstate, SDL_Event *event) {
   ...
   if (event->type == SDL_EVENT_PEN_MOTION) {
      /* you can check for when the pen is touching, but if pressure > 0.0f,
       * it's definitely touching! */
      if (pressure > 0.0f) {
         if (previous_touch_x >=
             0.0f) { /* only draw if we're moving while touching */
            ...
            SDL_RenderLine(renderer, previous_touch_x, previous_touch_y,
      ...
      if (event->paxis.axis == SDL_PEN_AXIS_PRESSURE) {
         pressure =
             event->paxis.value; /* remember new pressure for later draws. */
