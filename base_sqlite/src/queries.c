...
// CreatePerson :one
int createPerson_cb(sqlite3 *db, CreatePersonParams *params,
                    void (*cb)(Person *, void *), void *ctx) {
   const char *sql =
   ...
   sqlite3_bind_text(stmt, 1, (char *)params->name.data, params->name.len,
                     SQLITE_STATIC);
...
Person *createPerson(sql_allocator a, sqlite3 *db, CreatePersonParams *params) {
   ...
   createPerson_cb(db, params, createPerson_collect, &c);
   return c.out;
}

// GetPerson :one
int getPerson_cb(sqlite3 *db, sql_int64 id, void (*cb)(Person *, void *),
                 void *ctx) {
...
Person *getPerson(sql_allocator a, sqlite3 *db, sql_int64 id) {
   ...
   getPerson_cb(db, id, getPerson_collect, &c);
   return c.out;
}
...
static void getPeople_collect(Person *row, void *vctx) {
   ...
   if (c->len == c->cap) {
      ...
      if (c->items)
         memcpy(ni, c->items, c->len * sizeof(Person));
      c->items = ni;
      c->cap = ncap;
   }
...
PersonList getPeople(sql_allocator a, sqlite3 *db) {
   ...
   getPeople_cb(db, getPeople_collect, &c);
...
// CreatePet :one
int createPet_cb(sqlite3 *db, CreatePetParams *params,
                 void (*cb)(Pet *, void *), void *ctx) {
   const char *sql = "insert into pet (name, owner_id) values (:name, "
                     ":owner_id) returning *;";
   sqlite3_stmt *stmt;
   int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
   if (rc != SQLITE_OK)
...
Pet *createPet(sql_allocator a, sqlite3 *db, CreatePetParams *params) {
   ...
   createPet_cb(db, params, createPet_collect, &c);
   return c.out;
}
...
Pet *getPet(sql_allocator a, sqlite3 *db, sql_int64 id) {
   getPet_ctx c = {a, NULL};
   getPet_cb(db, id, getPet_collect, &c);
   return c.out;
}

// GetPetsByOwner :many
int getPetsByOwner_cb(sqlite3 *db, sql_int64 owner_id,
                      void (*cb)(Pet *, void *), void *ctx) {
...
static void getPetsByOwner_collect(Pet *row, void *vctx) {
   ...
   if (c->len == c->cap) {
      ...
      if (c->items)
         memcpy(ni, c->items, c->len * sizeof(Pet));
      c->items = ni;
      c->cap = ncap;
   }
...
PetList getPetsByOwner(sql_allocator a, sqlite3 *db, sql_int64 owner_id) {
   ...
   getPetsByOwner_cb(db, owner_id, getPetsByOwner_collect, &c);
   return (PetList){c.items, c.len};
}

// UpdatePet :one
int updatePet_cb(sqlite3 *db, UpdatePetParams *params,
                 void (*cb)(Pet *, void *), void *ctx) {
...
Pet *updatePet(sql_allocator a, sqlite3 *db, UpdatePetParams *params) {
   ...
   updatePet_cb(db, params, updatePet_collect, &c);
   return c.out;
}
