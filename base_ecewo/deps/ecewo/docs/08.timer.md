...
ecewo provides JavaScript-like `ecewo_timeout()` and `ecewo_interval()` functions, implemented with the libuv event loop, for non-blocking time-based operations.
...
The `ecewo_timeout()` function executes a callback function once after a specified delay.

**Signature:**
```c
Timer *ecewo_timeout(timer_callback_t callback, uint64_t delay_ms, void *user_data);
```
...
void on_user_register(const char *username) {
  ...
  ecewo_timeout(show_welcome_notification, 2000, name_copy);
...
session_timers_t* setup_session_timeout(int user_id) {
  ...
  *uid = user_id;
  timers->warning_timer = ecewo_timeout(warn_session_expiring, 25 * 60 * 1000, uid);
  
  // Expire after 30 minutes
  timers->expiry_timer = ecewo_timeout(expire_user_session, 30 * 60 * 1000, timers);
...
void cancel_session_timeout(session_timers_t *timers) {
  if (timers->warning_timer) {
    ecewo_clear_timer(timers->warning_timer);
    timers->warning_timer = NULL;
  }

  if (timers->expiry_timer) {
    ecewo_clear_timer(timers->expiry_timer);
...
The `ecewo_interval()` function executes a callback function repeatedly at specified intervals.

**Signature:**
```c
Timer *ecewo_interval(timer_callback_t callback, uint64_t interval_ms, void *user_data);
```
...
**Important:** For `ecewo_interval`, you **MUST call `ecewo_clear_timer()`** to stop the timer. Otherwise, it runs forever (until server shutdown).
...
Timer *start_time_display(void) {
  return ecewo_interval(print_current_time, 10000, NULL);
}

// Stop the timer
void stop_time_display(Timer *timer) {
  ecewo_clear_timer(timer);
}
```
...
auto_save_context_t* start_auto_save(int user_id, const char *doc_id) {
  ...
  ctx->auto_save_timer = ecewo_interval(auto_save_document, 30000, ctx);
...
void stop_auto_save(auto_save_context_t *ctx) {
  if (ctx) {
    if (ctx->auto_save_timer) {
      ecewo_clear_timer(ctx->auto_save_timer);
...
The `ecewo_clear_timer()` function cancels a pending or active timer.
...
- `timer`: Timer handle to cancel (from `ecewo_timeout` or `ecewo_interval`)

**When to Use:**

- **Always** for `ecewo_interval` timers (to stop repetition)
- For `ecewo_timeout` timers **only if cancelling before they fire**
- **Never** for `ecewo_timeout` timers after they've already fired (auto-cleaned)
...
void start_hello_timer(void) {
  hello_timer = ecewo_interval(say_hello, 2000, NULL);
  printf("Started saying hello every 2 seconds\n");
}

void stop_hello_timer(void) {
  if (hello_timer) {
    ecewo_clear_timer(hello_timer);
    hello_timer = NULL;
...
email_schedule_t* schedule_email(const char *to, const char *subject, 
  ...
  schedule->timer = ecewo_timeout(send_email, delay_ms, schedule);
...
void cancel_email(email_schedule_t *schedule) {
  if (schedule && schedule->timer) {
    ecewo_clear_timer(schedule->timer);
...
> [!NOTE]
>
> For `ecewo_timeout`, you **do NOT need to call `ecewo_clear_timer()`**. The timer automatically cleans up after executing. Only call `ecewo_clear_timer()` if you need to cancel before it fires.
>
> If you cancel with `ecewo_clear_timer()`, **you must free** user_data yourself

```c
// Normal execution
char *data = strdup("hello");
ecewo_timeout(callback, 1000, data);
// callback will free(data)

// Cancelled execution
char *data = strdup("hello");
Timer *timer = ecewo_timeout(callback, 1000, data);
ecewo_clear_timer(timer);
...
int main(void) {
  ...
  ecewo_interval(check_service_health, 30000, &db_check);
...
void handle_request(int user_id) {
  ...
  if (limit->request_count == 1) {
    ...
    *limit_copy = *limit;
    ecewo_timeout(reset_rate_limit, 60000, limit_copy);
  }
}
```
