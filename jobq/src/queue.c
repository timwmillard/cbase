...
int jobq_claim(sqlite3 *db, const char *worker_name, jobq_job *out)
{
    ...
    if (rc == SQLITE_DONE) { sqlite3_finalize(st); return 0; }   /* empty */
...
static int64_t backoff_secs(int attempt)
{
    ...
    int64_t s = 15;
    for (int i = 1; i < attempt && s < 3600; i++) s *= 2;
...
int jobq_fail(sqlite3 *db, const jobq_job *job, const char *err)
{
    ...
    sqlite3_bind_text (st, 2, retry ? "available" : "discarded", -1, SQLITE_STATIC);
...
{
    if (!j) return;
    free(j->kind); j->kind = NULL;
    free(j->args); j->args = NULL;
}
